...
class YoutubeMusicDesktopExtension extends Extension {
    ...
    async refreshPlaybackState() {
        ...
        const status = state && typeof state.playing === 'boolean'
    ...
    async postQuery(command) {
        ...
        try {
            ...
            if (['track-status', 'track-play-pause', 'track-play', 'track-pause', 'track-repeat', 'track-shuffle', 'track-forward', 'track-backward'].includes(command)) {
                this.refreshPlaybackState();
            }
            return response;
        } catch (error) {
